...
def run_detect(obj_center_x, obj_center_y, labels, edge_tpu, interpreter, input_mean, input_std, imW, imH, min_conf_threshold, output_details):
    ...
    # Normalize pixel values if using a floating model (i.e. if model is non-quantized)
        ...
        scores = interpreter.get_tensor(output_details[2]['index'])[0]
         ...
         # Initialize variables to track the person with the highest confidence score
        max_confidence = 0
        best_person_index = -1
        ...
        for i in range(len(scores)):
            ...
            # Draw rectangle and track the person with the highest confidence score
        if best_person_index != -1:
            ymin = int(max(1, (boxes[best_person_index][0] * imH)))
            xmin = int(max(1, (boxes[best_person_index][1] * imW)))
            ymax = int(min(imH, (boxes[best_person_index][2] * imH)))
            xmax = int(min(imW, (boxes[best_person_index][3] * imW)))
